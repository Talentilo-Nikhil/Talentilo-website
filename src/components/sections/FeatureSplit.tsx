...
export type FeatureSplitProps = {
  id?: string;
  /** The small caps kicker above the heading, e.g. "Smart Cadences". */
  eyebrow?: ReactNode;
  title: string;
  body: string;
  points: string[];
      ...
      <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-x-split">
        <div className={cn('flex flex-col gap-6', mediaSide === 'left' && 'lg:order-2')}>
          ...
          {pullQuote ? (
            <p
              className={cn(
                'max-w-[549px] border-l-2 pl-4 text-lede font-medium',
                dark ? 'border-brand-violet text-white' : 'border-ink/20 text-ink'
              )}
            >
              {pullQuote}
            </p>
