  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-baseline justify-between gap-4">
        <label htmlFor={id} className="text-body text-ink/85">
          {label}
        </label>
        <output htmlFor={id} className="font-figure text-h5 font-semibold text-ink">
          {display}
          {suffix ? <span className="text-body font-normal text-ink/70"> {suffix}</span> : null}
        </output>
      </div>
      <input
        id={id}
        ...
        className="h-2 w-full cursor-pointer appearance-none rounded-full bg-hairline
...
export function RoiCalculator({ seatCost }: RoiCalculatorProps) {
  ...
  return (
    <div className="grid gap-12 lg:grid-cols-2 lg:gap-20">
      ...
      <div className="flex flex-col justify-center gap-6 rounded-card bg-surface-tint p-8 lg:p-10">
